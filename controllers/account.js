...
const handleAccount = (knex, pdfjsLib) => (req, res) => {
    if (req.body.deleteAccount !== 'true') {
        ...
        handleAccountDelete(req, res, knex);
    }
}
...
const handleAccountDelete = async (req, res, knex) => {
    ...
    .then(function() {
        req.session.destroy(function (err) {
            if (err) {
                console.log('error logging out during account deletion: ', err);
                throw new Error('delete-logout-error');
            } else {
                // everything successful
                res.render('pages/error', { error: 'account-delete', user: null });
            }
        });
    })
    .catch(function(e) {
        console.error(e);
        if (e.message === 'delete-logout-error') {
            req.session.notification = {
                type: 'error',
                message: 'Error! Account deleted but could not log out properly.'
            }
            res.redirect('/');
        } else {
            ...
            req.session.notification = {
                type: 'error',
                message: 'Error deleting account. Something went wrong on our end :('
            };
            res.redirect('/account');
        }
    });
